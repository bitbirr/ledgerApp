...
import { Trees, Plus, Edit, Trash2, Lock, ChevronRight, ChevronDown, Building2 } from 'lucide-react';
...
export function ChartOfAccounts() {
  ...
  const renderAccountNode = (account: GLAccount, level: number, childrenMap: Map<string, GLAccount[]>) => {
        ...
        <TableRow className={`${level > 0 ? 'bg-muted/30' : ''}`}>
          <TableCell className="font-medium">
            <div className="flex items-center gap-2" style={{ paddingLeft: `${level * 20}px` }}>
              ...
              {isSystemAccount && <Lock className="h-3 w-3 text-muted-foreground" />}
            </div>
          </TableCell>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={() => deleteAccountMutation.mutate(account.id)}>
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
  ...
  const { rootAccounts, childrenMap } = glAccounts ? buildAccountTree(glAccounts) : { rootAccounts: [], childrenMap: new Map() };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pb-20 p-4 space-y-4">
          ...
          <CardContent>
            {isLoading ? (
              ...
              <Table>
                ...
                <TableBody>
                  {rootAccounts.map(account => renderAccountNode(account, 0, childrenMap))}
                </TableBody>
              </Table>
            )}
          </CardContent>
        ...
        <Dialog open={showAddAccount || !!editingAccount} onOpenChange={(open) => {
          if (!open) {
            setShowAddAccount(false);
            setEditingAccount(null);
            form.reset();
                    ...
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          ...
                          {glAccounts?.filter(acc => !acc.isLeaf).map(account => (
