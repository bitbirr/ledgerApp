...
import { Loader2, MapPin, Phone, Mail, Globe, Edit } from 'lucide-react';
...
const branchSchema = z.object({
  ...
  description: z.string().optional().or(z.literal('')), // allow empty string from defaultValues
...
export function EditBranchModal({ isOpen, onClose, branch, businessId }: EditBranchModalProps) {
  ...
  const updateBranchMutation = useMutation({
    mutationFn: async (data: BranchFormData) => {
      ...
      return api.updateBranch(branch.id, payload);
    },
    onSuccess: () => {
  ...
  const isSubmitting = updateBranchMutation.isPending;

  const onSubmit = async (data: BranchFormData) => {
    await updateBranchMutation.mutateAsync(data);
  };
